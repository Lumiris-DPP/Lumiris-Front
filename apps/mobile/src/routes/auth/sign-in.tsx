...
function SignInForm() {
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            navigate(returnTo ?? (isSignup ? '/onboarding/profile' : '/'));
        } catch (err: unknown) {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {isSignup ? (
                <p className="text-muted-foreground text-center text-[11px] leading-relaxed">
                    ...
                    <Link to="/about#cgu" className="text-foreground underline-offset-4 hover:underline">
                        CGV
                    </Link>{' '}
                    et la{' '}
                    <Link to="/about#rgpd" className="text-foreground underline-offset-4 hover:underline">
