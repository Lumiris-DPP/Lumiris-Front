...
import { signalRepairerInterest, type PublicRepairerDto, type RepairerReviewDto } from '@/lib/public-repairer-api';
...
export function RepairerProfile({ repairer, reviews: initialReviews }: RepairerProfileProps) {
    ...
    function handleSignalInterest() {
        ...
        signalRepairerInterest(repairer.id)
            .then(() => {
    ...
    const requestHref = useMemo(() => routes.repairerRequest(repairer.id, forParam), [repairer.id, forParam]);
        ...
        <div className="flex h-full flex-col overflow-y-auto bg-background pb-24">
            ...
            <div className="flex flex-col gap-5 px-4">
                ...
                {repairer.claimed ? (
                    ...
                    <section className="flex flex-col items-center gap-2 rounded-3xl border border-lumiris-amber/30 bg-lumiris-amber/5 p-5 text-center">
                        ...
                        <p className="text-xs leading-relaxed text-muted-foreground">
                            On ne peut pas encore lui envoyer de demande directement. Signale-nous que tu le cherches,
                            on l&apos;invitera à nous rejoindre.
                        </p>
                        <Button
                            type="button"
                            size="sm"
                    ...
                    <section className="flex flex-col gap-3">
                        <div className="flex items-center justify-between">
                            ...
                            {!showReviewForm ? (
                                <button
                                    ...
                                    className="inline-flex items-center gap-1 text-[11px] font-medium text-lumiris-cyan"
                                >
                        ...
                        {showReviewForm ? (
                            <form
                                ...
                                className="flex flex-col gap-3 rounded-2xl border border-border/60 bg-card p-3"
                            >
                                    ...
                                    {[1, 2, 3, 4, 5].map((value) => (
                                        ...
                                        >
                                            <Star
                                                className={`h-5 w-5 ${
                                                    value <= rating
                                                        ? 'fill-current text-lumiris-amber'
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <textarea
                                        ...
                                        className="rounded-xl border border-border bg-background px-3 py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground/60 focus:ring-2 focus:ring-lumiris-cyan/30"
                                    />
                                </div>
                                {reviewError ? (
                        ...
                        {reviews.length > 0 ? (
                                    ...
                                    <li key={r.id} className="rounded-2xl border border-border/60 bg-card p-3">
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <span className="inline-flex items-center gap-1 text-xs">
                                                <Star className="h-3 w-3 fill-current text-lumiris-amber" />
                                                {r.rating}
                                            </span>
                                        </div>
                                        {r.comment ? (
                            ...
                            <p className="text-xs text-muted-foreground/70 italic">Aucun avis pour l&apos;instant.</p>
                        ) : null}
                    </section>
                ) : null}
            </div>
